...
import abfold.np.residue_constants as residue_constants
from abfold.data.errors import MultipleChainsError
...
def parse(
    *, file_id: str, mmcif_string: str, catch_all_errors: bool = True
    ...
    try:
        ...
        valid_chains = _get_protein_chains_fake(parsed_info=parsed_info)
        if not valid_chains:
        ...
        for atom in _get_atom_site_list(parsed_info):
            if atom.model_num != "1":
                # We only process the first abfold at the moment.
                continue
...
def _get_first_model(structure: PdbStructure) -> PdbStructure:
    """Returns the first abfold in a Biopython structure."""
...
def _get_header(parsed_info: MmCIFDict) -> PdbHeader:
    ...
    if "_pdbx_audit_revision_history.revision_date" in parsed_info:
        ...
        header["release_date"] = '2021-07-01'
        # logging.warning(
...
def _get_protein_chains_fake(
    *, parsed_info: Mapping[str, Any]
) -> Mapping[ChainId, Sequence[Monomer]]:
    """this function is designed for this project, and it may be not suitable
    for similar projects.
    """
    valid_chains = collections.defaultdict(list)
    label_comp_id = parsed_info['_atom_site.label_comp_id']
    label_asym_id = parsed_info['_atom_site.label_asym_id']
    label_seq_id = parsed_info['_atom_site.label_seq_id']

    seq_id = 0
    for i, s_id in enumerate(label_seq_id):
        if seq_id != s_id:
            seq_id = s_id
            valid_chains[label_asym_id[i]].append(
                Monomer(
                    id=label_comp_id[i],
                    num=int(s_id),
                )
