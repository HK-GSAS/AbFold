...
def get_seq_from_pdb_str(pdb_str, chain_id):
    ...
    if len(models) != 1:
        raise ValueError(
            f"Only single abfold PDBs are supported. Found {len(models)} models."
        )
    model = models[0]
...
def from_pdb_string(pdb_str: str, chain_id: Optional[str] = None,
                    align: Optional[str] = None, seq: Optional[str] = None) -> Protein:
    ...
    for chain in model:
        ...
        if align is not None:
            j = 0
            for i, res_name in enumerate(align):
...
def to_pdb(prot: Protein) -> str:
    ...
    a = residue_constants.restype_num
    b = aatype > residue_constants.restype_num
    if np.any(b):
        raise ValueError(f"Invalid aatypes.")
    ...
    chain_tags = 'HLABCDEFGIJKMNOPQRSTUVWXYZ'
    # Add all atom sites.
    for i in range(n):
