...
def t_global_loss(
        pred_t_global: torch.Tensor,
        batch: Dict[str, torch.Tensor],
) -> torch.Tensor:
    atoms_N_CA_C_pos, _ = torch.split(batch['atom14_gt_positions'], [3, 11], -2)
    mean_atoms_N_CA_C_pos = torch.sum(atoms_N_CA_C_pos, -3) / torch.sum(batch['backbone_rigid_mask']).data
    point1, point2, point3 = torch.split(mean_atoms_N_CA_C_pos, [1, 1, 1], -2)
    gt_t_global = Rigid.from_3_points(point1, point2, point3).to_tensor_7()
    loss_fn = torch.nn.MSELoss()
    return loss_fn(pred_t_global, gt_t_global)
    ...
    def forward(self, out, batch, _return_breakdown=False):
        ...
        for loss_name, loss_fn in loss_fns.items():
            ...
            if (torch.isnan(loss) or torch.isinf(loss)):
                ...
                logging.warning(f"supervised {loss_name} loss is NaN. Skipping...")
        ...
        # loss_t = t_global_loss(out["sm"]["t_global_tensor_7"], batch)
        # scaled_loss_t = self.config['t_global'].weight * loss_t / torch.sqrt(min(seq_len, crop_len))
        # cum_loss = cum_loss + scaled_loss_t
